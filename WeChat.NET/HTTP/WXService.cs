...
namespace WeChat.NET.HTTP
{
    ...
    class WXService
    {
        ...
        private static DBHelper dh = new DBHelper("Default");

        private static string _addroom_msg_json = "{{\"BaseRequest\": {{\"Uin\": {0},\"Sid\": \"{1}\",\"Skey\": \"{2}\",\"DeviceID\": \"e441551176\"}},\"MemberCount\": {3},\"MemberList\": {4},\"Topic\": \"\"}}";

        private static string _delmem_msg_json = "{{\"BaseRequest\": {{\"Uin\": {0},\"Sid\": \"{1}\",\"Skey\": \"{2}\",\"DeviceID\": \"e441551176\"}},\"ChatRoomName\": \"{3}\",\"DelMemberList\": \"{4}\"}}";

        private static string _addmem_msg_json = "{{\"BaseRequest\": {{\"Uin\": \"{0}\",\"Sid\": \"{1}\",\"Skey\": \"{2}\",\"DeviceID\": \"e441551176\"}},\"ChatRoomName\": \"{3}\",\"AddMemberList\": \"{4}\"}}";
        ...
        public JObject WxSync()
        {
            ...
            if (sid != null && uin != null)
            {
                ...
                //dh.ExecuteNonQuery(string.Format("insert into WxSyncLog values('{0}')", sync_str.Replace("'","''")));
        ...
        public JObject AddMember(string roomName, List<string> userNames)
        {
            Cookie sid = BaseService.GetCookie("wxsid");
            Cookie uin = BaseService.GetCookie("wxuin");
            byte[] bytes = BaseService.SendPostRequest(string.Format(_delmember_url, LoginService.Pass_Ticket)
                , string.Format(_addmem_msg_json, uin.Value, sid.Value, LoginService.SKey, roomName
